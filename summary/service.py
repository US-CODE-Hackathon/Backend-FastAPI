...
from summary.model import Summary
...
def summarize_pattern(db: Session, idx: int):
    ...
    merged = "\n".join([f"{e.title}\n{e.sumary}" for e in entries])
    analysis = call_gemini_api(merged, idx, percent_str)
    return {"positive_ratio": percent_str, "pattern_summary": analysis}


# 최근 7개 summary(=diary) 데이터 가져오기
def get_recent_entries(db: Session, count: int):
    return db.query(Summary).order_by(Summary.date.desc()).limit(count).all()
...
def call_gemini_api(merged_text: str, count: int, percent_str: str) -> str:
    prompt = f"""
아래는 최근 {count}일간의 일기 제목과 요약입니다. 이 데이터를 바탕으로 전문 심리상담가가 작성하는 '정서 진단서' 형식으로, 다음 항목을 포함해 상세하게 분석해 주세요.
